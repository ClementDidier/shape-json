...
var defined = {
  ...
  foreach: require('./plugins/foreach')
};
...
function parse(provider, scheme) {
  ...
  let res = _.pickBy(json, predicate => _.isBoolean(predicate) || _.isNumber(predicate) || (!_.isEmpty(predicate) && !_.isNil(predicate)));
  return res;
};
  ...
  if (_.isString(text)) {
    ...
    if (data && data.length > 0) {
      ...
      for (let variable of unvalidatedVars) {
        if (variable.match(/^[a-zA-Z]+\d*(?:\.[a-zA-Z]+\d*)*$/g)) {
          vars.push(variable);
        }
      }
    }
